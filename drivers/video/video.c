...
{
    video->phys = (uintptr_t)mode_info->PhysBasePtr;
...
void video_clear(Video *video) {
    memset(video->data, 0, video->bytes_per_scanline * video->height);
}
